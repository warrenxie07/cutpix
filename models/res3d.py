...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv3d(inplanes, first_planes,
        ...
        self.drop_block = drop_block() if drop_block is not None else nn.Identity()
        self.act2 = act_layer(inplace=True)
        self.aa = create_aa(aa_layer, channels=width,
...
def drop_blocks(drop_prob=0.0):
    return [
        None,
        None,
        partial(DropBlock2d, drop_prob=drop_prob,
                block_size=5, gamma_scale=0.25)
        if drop_prob
        else None,
        partial(DropBlock2d, drop_prob=drop_prob,
                block_size=3, gamma_scale=1.00)
        if drop_prob
        else None,
    ]
...
def wide_res3d50_2(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 6, 3], base_width=128, **kwargs)
...
def wide_res3d101_2(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[
                      3, 4, 23, 3], base_width=128, **kwargs)
