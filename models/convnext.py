...
class ConvMlp(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Conv3d(in_features, hidden_features,
                             kernel_size=1, bias=True)
        self.norm = norm_layer(
            hidden_features) if norm_layer else nn.Identity()
        self.act = act_layer()
        self.fc2 = nn.Conv3d(hidden_features, out_features,
                             kernel_size=1, bias=True)
...
class LayerNorm3d(nn.LayerNorm):
    r"""LayerNorm for channels_first tensors with 2d spatial dimensions (ie N, C, D, H, W)."""

    def __init__(self, normalized_shape, eps=1e-6):
        super().__init__(normalized_shape, eps=eps)

    def forward(self, x) -> torch.Tensor:
        if _is_contiguous(x) and x.dim() == 5:
            return F.layer_norm(
                x.permute(0, 2, 3, 4, 1),
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        self.gamma = (
            nn.Parameter(ls_init_value * torch.ones(dim)
    ...
    def __init__(
        ...
        if in_chs != out_chs or stride > 1:
            self.downsample = nn.Sequential(
                norm_layer(in_chs),
                nn.Conv3d(in_chs, out_chs, kernel_size=stride, stride=stride),
            )
        else:
    ...
    def __init__(
        ...
        if stem_type == "patch":
            self.stem = nn.Sequential(
                nn.Conv3d(
                    in_chans, dims[0], kernel_size=patch_size, stride=patch_size),
                norm_layer(dims[0]),
            )
        ...
        for i in range(4):
            ...
            self.feature_info += [
                dict(num_chs=prev_chs, reduction=curr_stride,
                     module=f"stages.{i}")
            ]
        ...
        self.head = nn.Sequential(
            OrderedDict(
                [
                    ("global_pool", Reduce("b c d h w -> b c", "mean")),
                    (
                        "norm",
                        nn.Identity()
                        if head_norm_first
...
def convnext_nano_hnf(pretrained=False, **kwargs):
    ...
    model = _create_convnext(
        "convnext_nano_hnf", pretrained=pretrained, **model_args)
    return model
...
def convnext_tiny_in22k(pretrained=False, **kwargs):
    model_args = dict(depths=(3, 3, 9, 3), dims=(96, 192, 384, 768), **kwargs)
    model = _create_convnext("convnext_tiny_in22k",
...
if __name__ == "__main__":
    net = convnext_tiny(num_classes=19, in_chans=1, patch_size=2)
